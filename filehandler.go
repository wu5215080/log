...
func NewTimeRotatingFileHandler(baseName string, when int8, interval int) (*TimeRotatingFileHandler, error) {
	...
	strtimeformat := "2006-01-02 15:04:05"
	strdate := filetime.Format("2006-01-02")
	switch when {
	case WhenHour:	
		strtime := strdate+" "+strconv.Itoa(filetime.Hour()) +":59:59"
		t, _ := time.ParseInLocation(strtimeformat, strtime, time.Local)
		h.rolloverAt = t.Unix()+1
	case WhenDay:
		//获取23:59:59秒的时间戳，用于第一次分割日志文件,第一次分割后，按照周期interval分割
		//获取23:59:59秒的时间戳
		t, _ := time.ParseInLocation(strtimeformat, strdate+" 23:59:59", time.Local)  
		h.rolloverAt = t.Unix() + 1
	default:
		h.rolloverAt = filetime.Unix()  + h.interval
	}
	return h, nil
}
